...
if (isset($_POST['addupdate'])) 
{
	...
	if ($input_error != 1)
	{
		...
		{
			$filename = '/images/'.item_img_name($_POST['NewStockID']).".jpg";
			if (file_exists($filename))
				unlink($filename);
		}
...
if (isset($_POST['delete']) && strlen($_POST['delete']) > 1) 
{
	...
	if (check_usage($_POST['NewStockID'])) {
		...
		$filename = '/images/'.item_img_name($stock_id).".jpg";
		if (file_exists($filename))
			unlink($filename);
