<?php
	...
	function add_gl_tax_details($gl_code, $trans_type, $trans_no, $amount, $ex_rate, $date, $memo) {
		...
		add_trans_tax_details(
			$trans_type, $trans_no, $tax['id'], $tax['rate'], 0,
	...
	function write_journal_entries(&$cart, $reverse, $use_transaction = true) {
		...
		foreach (
			$cart->gl_items as $journal_item
		)
		{
			// post to first found bank account using given gl acount code.
			$is_bank_to = Banking::is_bank_account($journal_item->code_id);
			add_gl_trans(
				$trans_type, $trans_id, $date_, $journal_item->code_id,
		...
		if ($reverse) {
			...
			)
			{
				$is_bank_to = Banking::is_bank_account($journal_item->code_id);
				add_gl_trans(
					$trans_type, $trans_id_reverse, $reversingDate,
