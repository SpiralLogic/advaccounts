<?php
	...
	if ($settings['concatenate']) {
		if (count(array_unique($fileTypes)) > 1) {
			debugExit("Files must be of the same type.");
		}
	}
	...
	if ($settings['clientCache'] && $settings['clientCacheCheck']) {
		if ($settings['serverCache'] && !$generateContent) {
			$mtime = filemtime($cachedFile);
		}
		elseif ($settings['serverCache']) {
			$mtime = time();
		}
		else {
			$mtime = filesmtime();
	...
	if (!$settings['clientCache'] || !$settings['clientCacheCheck'] || !isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) || $_SERVER['HTTP_IF_MODIFIED_SINCE'] != $mtimestr) {
		...
		if ($generateContent) {
			if ($settings['minify' . $fileType]) {
				include('minifiers/' . $fileType . '.php');
			}
			$content = array();
			foreach ($files as $file) {
				(($content[] = @file_get_contents($file)) !== false) || debugExit("File not found ($file).");
			}
			$content = implode("\n", $content);
			if ($settings['minify' . $fileType]) {
				$content = call_user_func('minify_' . $fileType, $content);
			}
			if ($settings['gzip']) {
