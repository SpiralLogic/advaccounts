...
$txt .= "&nbsp;&nbsp;Y <input type='text' name='y1' size='4' maxlength='4' value='" . (isset($y1) ? $y1
...
table_header($th);
start_row();
$txt = "<input type='radio' name='sel_typ' value = 'String' " . (empty($sel_typ) ? " checked "
		: m_c("String", $sel_typ)) . ">String\n";
label_cell($txt);
...
if (($report_type == "single") && (substr($id_new, 0, 1) != 'B')) { //  show Textarea-element, if Report and single_typed
	...
	$txt .= "</select>\n";
	$txt .= WIDTH . "<input type='text' name='width' size=3 maxlength=3 value='" . (isset($width) ? $width
			: "") . "' onBlur='num_test(this);'>\n";
	label_cell($txt);
	end_row();
}
if (substr($id_new, 0, 1) != 'B') { // This is a report -> show block
	start_row();
