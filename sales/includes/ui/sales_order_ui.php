	...
	function display_order_header($order, $editable, $date_text, $display_tax_group = false)
		{
			...
			if (!Banking::is_company_currency($order->customer_currency)) {
				...
				Display::exchange_rate($order->customer_currency, Banking::get_company_currency(),
			...
			if ($change_prices != 0) {
				foreach ($order->line_items as $line_no => $item) {
					...
					$line->price = Item_Price::get_kit($line->stock_id, $order->customer_currency, $order->sales_type, $order->price_factor,
						get_post('OrderDate'));
	...
	function sales_order_item_controls($order, &$rowcounter, $line_no = -1)
		{
			...
			} else // prepare new line
			{
				...
				$price = Item_Price::get_kit(Input::post('stock_id'), $order->customer_currency, $order->sales_type, $order->price_factor,
					get_post('OrderDate'));
