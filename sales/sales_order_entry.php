<?php
	...
	Security::set_page((!Input::session('Items') ? : $_SESSION['Items']->trans_type), array(
		...
		'NewOrder' => 'SA_SALESORDER', 'ModifySalesOrder' => 'SA_SALESORDER', 'NewQuotation' => 'SA_SALESQUOTE', 'ModifyQuotationNumber' => 'SA_SALESQUOTE', 'NewDelivery' => 'SA_SALESDELIVERY', 'NewInvoice' => 'SA_SALESINVOICE'));
	...
	if (Input::get('NewInvoice', Input::NUMERIC)) {
		...
		if (isset($_SESSION['Items'])) {
			foreach ($_SESSION['remote_order']->line_items as $item) {
				Sales_Order::add_line($_SESSION['Items'], $item->stock_id, $item->quantity, $item->price, $item->discount_percent, $item->description);
			}
	...
	function page_complete($order_no, $trans_type, $trans_name = 'Transaction', $edit = false, $update = false)
	{
		...
		if ($trans_type == ST_SALESORDER) {
			...
			((isset($_GET['Type']) && $_GET['Type'] == 1)) ? submenu_option(_("Enter a New Template &Delivery"), "/sales/inquiry/sales_orders_view.php?DeliveryTemplates=Yes") : submenu_option(_("Enter a &New Delivery"), "/sales/sales_order_entry.php?NewDelivery=0");
	...
	function check_item_data()
	{
		if (!User::get()->can_access('SA_SALESCREDIT') && (!Validation::is_num('qty', 0) || !Validation::is_num('Disc', 0, 100))) {
			...
			JS::set_focus('price');
			return false;
		} elseif (!User::get()->can_access('SA_SALESCREDIT') && isset($_POST['LineNo']) && isset($_SESSION['Items']->line_items[$_POST['LineNo']]) && !Validation::is_num('qty', $_SESSION['Items']->line_items[$_POST['LineNo']]->qty_done)
		) {
			JS::set_focus('qty');
		...
		elseif ($_SESSION['Items']->trans_type != ST_SALESORDER && $_SESSION['Items']->trans_type != ST_SALESQUOTE && !SysPrefs::allow_negative_stock() && Item::is_inventory_item($_POST['stock_id'])) {
			...
			if (input_num('qty') > $qoh) {
				...
				Errors::error(_("The delivery cannot be processed because there is an insufficient quantity for item:") . " " . $stock['stock_id'] . " - " . $stock['description'] . " - " . _("Quantity On Hand") . " = " . Num::format($qoh, Num::qty_dec($_POST['stock_id'])));
				return false;
			}
			return true;
		}
	...
	function handle_new_item()
	{
		...
		Sales_Order::add_line($_SESSION['Items'], $_POST['stock_id'], input_num('qty'), input_num('price'), input_num('Disc') / 100, $_POST['description']);
