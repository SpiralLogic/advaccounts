    ...
    switch ($row['type']) {
        ...
        case ST_CUSTCREDIT:
            if (get_voided_entry(ST_CUSTCREDIT, $row["trans_no"]) === false &&
                $row['Allocated'] == 0) // 2008-11-19 Joe Hunt
            {
...
if (isAjaxReferrer() && !empty($_POST['ajaxsearch'])) {
    ...
    foreach ($searchArray as $ajaxsearch) {
        ...
        ;
       if (stripos($ajaxsearch,'/')>0) $sql .= " tran_date LIKE '%" . date2sql($ajaxsearch, false)."%' OR";
...
$cols = array(_("Type") => array('fun' => 'systype_name', 'ord' => ''),
              ...
              _("Due Date") => array('type' => 'date', 'fun' => 'due_date'), _("Customer") => array('ord' => ''),
              ...
              array('insert' => true, 'fun' => 'credit_link'), array('insert' => true, 'fun' => 'edit_link'),
