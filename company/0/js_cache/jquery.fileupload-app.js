...
$(function () {
			  ...
			  fileUploadOptions = {
				  ...
				  buildDownloadRow: function (file) {
					  ...
					  return $('#template_download').clone().removeAttr('id').attr('data-file-id', encodedFileName).attr('data-id', file.id).find('.file_upload_name a').text(file.name)
							  .attr({'href': 'upload/' + encodedFileName + '?id=' + file.id,'target':'_blank'}).closest('tr');
				  },
				  progressAllNode: $('#file_upload_progress div'),
				  beforeSend: function (event, files, index, xhr, handler, callBack) {
					  //   handler.uploadRow.find('.file_upload_start button').click(function () {
					  $(this).remove();
					  $.getJSON('upload.php?file=' + handler.uploadRow.attr('data-file-id'), function (file) {
	  ...
	  $.getJSON('upload.php?order=' + order_id, function (files) {
	  ...
	  $('#files .file_upload_delete button').live('click', function () {
		                                              ...
		                                              $.ajax('upload.php?file=' + row.attr('data-file-id') + '&id=' + row.attr('data-id'), {
			                                                     type: 'DELETE',
			                                                     success: function () {
	  ...
	  $('#file_upload').attr('action', 'upload.php').fileUploadUI(fileUploadOptions);
  });