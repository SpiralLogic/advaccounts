...
var Branches = function () {
	var current = {}, list = $("#branchList"), adding = false, btnAdd = $("#addBranch").button(), btnDel = $("#delBranch").button();
	return {
		init:function () {
			btnAdd.hide().removeClass('invis');
			btnDel.hide().removeClass('invis').click(function(){Branches.del()});
		...
		btnBranchAdd:function () {
			...
			if (!adding && current.branch_code > 0 && Customer.get().id > 0) {
				btnAdd.button('option', 'label', 'Add').one('click',
																														function (event) {
																															Branches.New();
																															adding = true;
																															return false
				...
				btnDel.show();
			} else if			(current.branch_code > 0) { btnAdd.show();btnDel.show(); }else{ btnDel.hide();btnAdd.hide();}


			return false;
		},
		del:function () {
			$.post('customers.php', {deletebranch:current.branch_code,id:Customer.get().id}, function (data) {
				if (data.status) {
					Adv.showStatus(data.status);
					if (!data.status.status) return;
					delete Customer.get().branches[current.id];
				}
				Adv.resetHighlights();
				adding = false;
			}, 'json');
...
$(function () {
	Adv.extend({
							 ...
							 $shortcutTabs:$("#tabs1").tabs({ select:function (event, ui) {
								 ...
								 if (url) {location.href = url + Customer.get().id;}
								 return false;
							 },
																								selected:-1
																							}),
							 ...
							 btnCancel:$("#btnCancel").button().click(function () {
								 ...
								 return false;
							 }), btnDelete:$("#delBranch").button().click(function () {
			(	!Adv.fieldsChanged > 0) ? Adv.resetState() : Adv.revertState();
			return false;
		}),
