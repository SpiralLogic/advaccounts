<?php
  ...
  class UndepositedFunds extends \ADV\App\Controller\Base
  {
    ...
    protected function reset() {
      ...
      $_POST['deposit_date']   = $this->check_date() ? (Input::post('deposit_date')) : '';
    ...
    protected function deposit() {
      ...
      $query       = $this->DB->_query($sql);
      $undeposited = [];
      while ($row = $this->DB->_fetch($query)) {
      ...
      if (count($togroup) > 1) {
        ...
        $sql      = "INSERT INTO bank_trans (type, bank_act, amount, ref, trans_date, person_type_id, person_id, undeposited) VALUES (" . ST_GROUPDEPOSIT . ", 5, $total_amount," . $this->DB->_quote('Group Deposit') . ",'" . $this->Dates->_dateToSql($_POST['deposit_date']) . "', 6, '" . User::i()->user . "',0)";
        $query    = $this->DB->_query($sql, "Undeposited Cannot be Added");
        $order_no = $this->DB->_insertId($query);
        if (!isset($order_no) || !empty($order_no) || $order_no == 127) {
          $sql      = "SELECT LAST_INSERT_ID()";
          $order_no = $this->DB->_query($sql);
          $order_no = $this->DB->_fetchRow($order_no);
          $order_no = $order_no[0];
        }
        foreach ($togroup as $row) {
          $sql = "UPDATE bank_trans SET undeposited=" . $order_no . " WHERE id=" . $this->DB->_escape($row['id']);
          $this->DB->_query($sql, "Can't change undeposited status");
        }
      } else {
        $row = reset($togroup);
        $sql = "UPDATE bank_trans SET undeposited=0, trans_date='" . $this->Dates->_dateToSql($_POST['deposit_date']) . "',deposit_date='" . $this->Dates->_dateToSql($_POST['deposit_date']) . "' WHERE id=" . $this->DB->_escape($row['id']);
        $this->DB->_query($sql, "Can't change undeposited status");
      }
    ...
    protected function index() {
      ...
      Display::div_end();
      echo "<hr>";
      $date         = $this->Dates->_addDays($_POST['deposit_date'], 10);
      $sql          = "SELECT	type, trans_no, ref, trans_date,
                    amount,	person_id, person_type_id, reconciled, id
            FROM bank_trans
            WHERE undeposited=1 AND trans_date <= '" . $this->Dates->_dateToSql($date) . "' AND reconciled IS null AND amount<>0
     ...
     */
    function check_date() {
      if (!$this->Dates->_isDate(Input::post('deposit_date'))) {
    ...
    function depositCheckbox($row) {
      $amount     = $row['amount'];
      $chk_value = Input::hasPost($row['id'])?'checked':'';
    ...
    function change_tpl_flag($deposit_id) {
      if (!$this->check_date() && Input::hasPost("dep_" . $deposit_id)) // temporary fix
      {
        return false;
      }
      if (Input::post('deposit_date') == $this->Dates->_today()) {
      ...
      if (!count($_SESSION['undeposited'])) {
        $_POST['deposit_date'] = $this->Dates->_today();
      }
      return true;
    }
