<?php
  ...
  ADVAccounting::i()->setSelected('Debtors');
  if (AJAX_REFERRER) {
    if (isset($_GET['term'])) {
  ...
  Table::section(1);
  Table::sectionTitle(_("Accounts Details:"), 2);
  Forms::percentRow(_("Discount Percent:"), 'discount', $customer->discount, (User::i()->can_access(SA_CUSTOMER_CREDIT)) ? "" :
    " disabled");
  Forms::percentRow(_("Prompt Payment Discount Percent:"), 'payment_discount', $customer->payment_discount, (User::i()
    ->can_access(SA_CUSTOMER_CREDIT)) ? "" : " disabled");
