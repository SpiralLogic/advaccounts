<?php
  ...
  class SalesPeople extends Base
  {
    ...
    protected function before() {
      list($this->Mode, $this->selected_id) = Page::simple_mode(true);
      ...
      if ($this->Mode == MODE_DELETE) {
        ...
        $sql    = "SELECT COUNT(*) FROM branches WHERE salesman=" . DB::_escape($this->selected_id);
        ...
        if ($myrow[0] > 0) {
          ...
          $sql = "DELETE FROM salesman WHERE salesman_code=" . DB::_escape($this->selected_id);
      ...
      $this->result = DB::_query($sql, "could not get sales persons");
    }
    protected function index() {
      ...
      Table::header($th);
      while ($myrow = DB::_fetch($this->result)) {
      ...
      if ($this->selected_id != -1) {
        if ($this->Mode == MODE_EDIT) {
          ...
          $sql                     = "SELECT * FROM salesman WHERE salesman_code=" . DB::_escape($this->selected_id);
        ...
        Forms::hidden('selected_id', $this->selected_id);
      ...
      Table::end(1);
      Forms::submitAddUpdateCenter($this->selected_id == -1, '', 'both');
      Forms::end();
      Page::end();
    }
  }
