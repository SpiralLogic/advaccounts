<?php
    ...
    if ($Mode == ADD_ITEM || $Mode == UPDATE_ITEM) {
        ...
        if ($input_error != 1) {
            if ($selected_id != -1) {
 			...
 			VALUES (" . DB::escape($_POST['salesman_name']) . ", " . DB::escape($_POST['user_id']) . ", " . DB::escape($_POST['salesman_phone']) . ", " . DB::escape(
                    $_POST['salesman_fax']
                ) . ", " . DB::escape($_POST['salesman_email']) . ", " . Validation::input_num('provision') . ", " . Validation::input_num(
                    'break_pt'
    ...
    while ($myrow = DB::fetch($result)) {
        ...
        Cell::label(Num::percentFormat($myrow["provision"]) . " %", ' class="alignright nowrap"');
        Cell::amount($myrow["break_pt"]);
        Cell::label(Num::percentFormat($myrow["provision2"]) . " %", ' class="alignright nowrap"');
    ...
    if ($selected_id != -1) {
        if ($Mode == MODE_EDIT) {
            ...
            $_POST['provision']      = Num::percentFormat($myrow["provision"]);
            $_POST['break_pt']       = Num::priceFormat($myrow["break_pt"]);
            $_POST['provision2']     = Num::percentFormat($myrow["provision2"]);
        }
        Forms::hidden('selected_id', $selected_id);
    } elseif ($Mode != ADD_ITEM) {
        $_POST['provision']  = Num::percentFormat(0);
        $_POST['break_pt']   = Num::priceFormat(0);
        $_POST['provision2'] = Num::percentFormat(0);
    }
