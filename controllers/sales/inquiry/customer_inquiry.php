<?php
  ...
  class SalesInquirey extends \ADV\App\Controller\Base {
    ...
    protected function before() {
      ...
      Input::$post['customer_id'] = Input::postGetGlobal('customer_id', INPUT::NUMERIC, null);
    ...
    protected function index() {
      ...
      Row::start();
      Debtor::newselect(null,['input_cell_params'=>['placeholder'=>'customer'],'label'=>false,'row'=>false]);
      ...
      if ($this->isAjaxSearch) {
        $sql = $this->prepareAjaxSearch();
      } else {
        $sql = $this->prepareSearch();
      }
      ...
      $cols = array(
        _("Type")                                              => array(
        ...
        ), _("Reference")                                      => array('ord' => ''), _("Date")                  => array('name' => 'tran_date', 'type' => 'date', 'ord' => 'desc'), _("Due Date")              => array(
          'type' => 'date', 'fun' => function ($row) {
            return $row["type"] == ST_SALESINVOICE ? $row["due_date"] : '';
          }
        ), _("Customer")                                       => array('ord' => 'asc'), array('type' => 'skip'), _("Branch")                => array('ord' => ''), _("Currency")              => array('align' => 'center', 'type' => 'skip'), _("Debit")                 => array(
        ...
        ), array('type' => 'skip'), _("RB")                    => array('align' => 'right', 'type' => 'amount'), array(
          'insert' => true, 'fun' => function ($row) {
            return GL_UI::view($row["type"], $row["trans_no"]);
          }
        ), array(
          'insert' => true, 'align' => 'center', 'fun' => function ($row) {
            return $row['type'] == ST_SALESINVOICE && $row["TotalAmount"] - $row["Allocated"] > 0 ? DB_Pager::link(_("Credit"), "/sales/customer_credit_invoice.php?InvoiceNumber=" . $row['trans_no'], ICON_CREDIT) : '';
          }
        ), array(
          'insert' => true, 'align' => 'center', 'fun' => function ($row) {
            return $row['type'] == ST_SALESINVOICE && $row["TotalAmount"] - $row["Allocated"] > 0 ? DB_Pager::link(_("Payment"), "/sales/customer_payments.php?customer_id=" . $row['debtor_id'], ICON_MONEY) : '';
          }
        ), array(
          'insert' => true, 'align' => 'center', 'fun' => function ($row) {
            $str = '';
          ...
          'insert' => true, 'align' => 'center', 'fun' => function ($row) {
            ...
            UI::button(false, 'Email', array(
                                            'class'        => 'button email-button', 'data-emailid' => $row['debtor_id'] . '-' . $row['type'] . '-' . $row['trans_no']
                                       ));
