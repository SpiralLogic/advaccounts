<?php
  ...
  function display_type($type, $typename, $yr, $mo, $convert, &$dec, &$rep, $dimension, $dimension2)
	...
	function print_annual_expense_breakdown()
	{
		...
		if ($dim == 2) {
			$params = array(
				0 => $comments, 1 => array(
					...
					'text' => _("Dimension") . " 2", 'from' => Dimensions::get_string($dimension2), 'to' => ''), 4 => array(
					'text' => _('Info'), 'from' => _('Amounts in thousands'), 'to' => ''));
		} else if ($dim == 1) {
			$params = array(
				0 => $comments, 1 => array(
					'text' => _("Year"), 'from' => $year, 'to' => ''), 2 => array(
					'text' => _('Dimension'), 'from' => Dimensions::get_string($dimension), 'to' => ''), 3 => array(
					'text' => _('Info'), 'from' => _('Amounts in thousands'), 'to' => ''));
		} else {
			$params = array(
				0 => $comments, 1 => array(
		...
		$rep = new ADVReport(_('Annual Expense Breakdown'), "AnnualBreakDown", User::page_size());
		$rep->Font();
