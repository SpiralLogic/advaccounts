<?php
	...
	include_once($_SERVER['DOCUMENT_ROOT'] . "/includes/ui/db_pager_view.inc");
			...
			if ($page) {
				...
				if ($page == 'next' && !$this->next_page ||
				 $page == 'last' && !$this->last_page
					...
					ui_view::set_focus($this->name . '_page_prev');
				if ($page == 'prev' && !$this->prev_page ||
				 $page == 'first' && !$this->first_page
				)
					ui_view::set_focus($this->name . '_page_next');
			} elseif ($sort != -1) {
	...
	function countFilter($table = false, $feild = false, $where = false) {
		if ($table && $where && $feild) {
			$sql = "SELECT * FROM " . $table . " WHERE " . $feild . " LIKE " . db_escape($where) . " LIMIT 1";
			$result = db_query($sql, 'Couldnt do shit');
