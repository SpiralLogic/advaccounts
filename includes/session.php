<?php
	...
	if (strstr($_SERVER['PHP_SELF'], 'logout.php') == false) {
		Login::timeout();
		if (!CurrentUser::instance()->logged_in()) {
			// Show login screen
			if (!isset($_POST["user_name_entry_field"]) or $_POST["user_name_entry_field"] == "") {
				...
				$succeed = (Config::get('db.' . $_POST["company_login_name"])) && CurrentUser::instance()->login($_POST["company_login_name"], $_POST["user_name_entry_field"], $_POST["password"]);
				// select full vs fallback ui mode on login
				CurrentUser::instance()->ui_mode = $_POST['ui_mode'];
				if (!$succeed) {
					// Incorrect password
					Login::fail();
				}
