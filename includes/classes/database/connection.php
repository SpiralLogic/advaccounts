 ...
 */
	...
	abstract class Database_Connection {
		...
		public static function instance($name = NULL, array $config = NULL) {
			...
			static::$db = $db_connections;
			if ($name === NULL) {
				// Use the default instance name
				$name = static::$db[0]['name'];
			}
			...
			if (!isset(static::$instances[$name])) {
				...
				if (!isset($config['type'])) {
					throw new Adv_Exception("Database type not defined in {$name} configuration");
				}

				// Set the driver class name
				add_include_path(APP_PATH . 'includes/database/' . ucfirst($config['type']) . '/connection.php');
