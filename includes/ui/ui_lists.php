...
JS
	...
	{
		$sql
		 = "SELECT branch_code, branch_ref FROM cust_branch
	...
	function payment_person_types_list($name, $selected_id = null, $submit_on_change = false)
	{
		...
		$items = array();
		foreach (
			$payment_person_types as $key => $type
		) {
			if ($key != PT_WORKORDER) {
				$items[$key] = $type;
			}
	...
	function pagesizes_list_row($label, $name, $value = null)
	{
		$items = array();
		foreach (
			Config::get('formats_paper_size') as $pz
		) {
			$items[$pz] = $pz;
		}
		echo "<tr><td class='label'>$label</td>\n<td>";
	...
	function journal_types_list_cells($label, $name, $value = null, $submit_on_change = false)
	{
		...
		foreach (
			array(ST_PURCHORDER, ST_WORKORDER, ST_SALESORDER, ST_DIMENSION, ST_SALESQUOTE) as $excl
		) {
			unset($items[$excl]);
		}
		echo array_selector(
