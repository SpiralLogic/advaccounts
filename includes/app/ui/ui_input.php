 ...
 */
	...
	function inactive_control_cell($id, $value, $table, $key) {
		...
		if (check_value('show_inactive')) {
			...
			echo '<td align="center">' . checkbox(null, $name, $value, true, '', "align='center'") . hidden("LInact[$id]", $value,
				false) . '</td>';
		}
	}
	...
	function customer_credit_row($customer, $credit, $parms = '') {
		label_row(_("Current Credit:"), "<a target='_blank' " . ($credit < 0 ? 'class="redfg openWindow"' :
		 '') . "href='/sales/inquiry/customer_inquiry.php?frame=1&customer_id=" . $customer . "'" . " >" . Num::price_format($credit) . "</a>",
			$parms);
	}

	function supplier_credit_row($supplier, $credit, $parms = '') {
		label_row(_("Current Credit:"), "<a target='_blank' " . ($credit < 0 ? 'class="redfg openWindow"' :
		 '') . "href='/purchases/inquiry/supplier_inquiry.php?supplier_id=" . $supplier . "' >" . Num::price_format($credit) . "</a>",
			$parms);
	}
	...
	function bank_balance_row($bank_acc, $parms = '') {
		...
		label_row(_("Bank Balance:"), "<a target='_blank' " . ($bal < 0 ? 'class="redfg openWindow"' :
		 '') . "href='/gl/inquiry/bank_inquiry.php?bank_account=" . $bank_acc . "'" . " >&nbsp;" . Num::price_format($bal) . "</a>",
			$parms);
	}
