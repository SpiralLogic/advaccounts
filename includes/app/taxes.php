<?php
	...
	class Taxes
	{
		...
		//
		public static function get_tax_free_price_for_item($stock_id, $price, $tax_group, $tax_included, $tax_group_array = null)
			{
		...
		//
		public static function get_tax_for_item($stock_id, $price, $tax_group = false)
			{
				// if price is zero, then can't be taxed !
				if ($price == 0) {
					return 0;
		...
		public static function get_full_price_for_item($stock_id, $price, $tax_group, $tax_included, $tax_group_array = null)
			{
				// if price is zero, then can't be taxed !
				if ($price == 0) {
					return 0;
		...
		static function edit_items($taxes, $columns, $tax_included, $leftspan = 0, $tax_correcting = false)
			{
				...
				foreach (
					...
					if ($tax_included) {
						label_row(_("Included") . " " . $taxitem['tax_type_name'] . " (" . $taxitem['rate'] . "%) " . _("Amount:") . " ",
							Num::format($taxitem['Value'], User::price_dec()), "colspan=$columns align=right", "align=right", $leftspan);
					}
					else {
				...
				if ($tax_correcting) {
					label_cell(_("Tax Correction"), "colspan=$columns align=right style='width:90%'");
