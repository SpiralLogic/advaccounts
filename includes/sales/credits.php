<?php
	...
	class Sales_Credits
	{
		function add($credit_note, $write_off_acc)
		{
			...
			$credit_no = Sales_Trans::write(ST_CUSTCREDIT, $trans_no, $credit_note->customer_id, $credit_note->Branch, $credit_date, $credit_note->reference, $credit_note_total, 0, $items_added_tax, $credit_note->freight_cost, $freight_added_tax, $credit_note->sales_type, $credit_note->order_no,
			...
			if ($credit_invoice) {
				...
				if ($invoice_alloc_balance > 0) { //the invoice is not already fully allocated
					...
					if ($allocate_amount != 0) {
						...
						Banking::exchange_variation(ST_CUSTCREDIT, $credit_no, ST_SALESINVOICE, $credit_invoice, $credit_date, $allocate_amount, PT_CUSTOMER);
						///////////////////////////////////////////////////////////////////////////
					}
				}
			}
			...
			foreach ($credit_note->line_items as $credit_line) {
				...
				Sales_Debtor_Trans::add(ST_CUSTCREDIT, $credit_no, $credit_line->stock_id, $credit_line->description, $credit_line->qty_dispatched, $credit_line->line_price(), $line_tax, $credit_line->discount_percent, $credit_line->standard_cost, $trans_no == 0 ? 0 : $credit_line->id);
				Sales_Credits::add_movements($credit_note, $credit_line, $credit_type, $line_taxfree_price + $line_tax, $credit_invoice);
				$total += Sales_Credits::add_gl_costs($credit_note, $credit_line, $credit_no, $credit_date, $credit_type, $write_off_acc, $branch_data);
		...
		//
		function add_movements(&$credit_note, &$credit_line, $credit_type, $price, $credited_invoice = 0)
		{
		...
		//----------------------------------------------------------------------------------------
		function add_gl_costs($order, $order_line, $credit_no, $date_, $credit_type, $write_off_gl_code, &$branch_data)
		{
			...
			if ($order_line->line_price() != 0) {
				$line_taxfree_price = Taxes::get_tax_free_price_for_item($order_line->stock_id, $order_line->price, 0, $order->tax_included, $order->tax_group_array);
				$line_tax = Taxes::get_full_price_for_item($order_line->stock_id, $order_line->price, 0, $order->tax_included, $order->tax_group_array) - $line_taxfree_price;
