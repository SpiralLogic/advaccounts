		...
		public static function add($supp_trans, $invoice_no = 0) // do not receive as ref because we change locally
			{
				...
				$ex_rate = Banking::get_exchange_rate_from_home_currency(Banking::get_supplier_currency($supp_trans->supplier_id),
					$date_);
				...
				foreach ($supp_trans->grn_items as $entered_grn)
				{
					...
					if ($supp_trans->is_invoice) {
						...
						$current_purchase_price = Item_Price::getPriceBySupplier(Item::getStockID($entered_grn->item_code),
							$supp_trans->supplier_id)->price;
						...
						$deliveries = Item::get_deliveries_between($entered_grn->item_code, $old_date,
							Dates::Today()); // extend the period, if invoice is before any deliveries.
						...
						Inv_Transfer::update_pid(ST_SUPPRECEIVE, $entered_grn->item_code, $old_date, $old_date, $supp_trans->supplier_id,
							$mat_cost);
						//}
					}
					// ----------------------------------------------------------------------
				...
				/* Now the TAX account */
				$taxes = $supp_trans->get_taxes($supp_trans->tax_group_id, 0,
					false); // 2009.08-18 Joe Hunt. We have already got the gl lines
				foreach ($taxes as $taxitem)
				{
					if ($taxitem['Net'] != 0) {
						if (!$supp_trans->is_invoice) {
		...
		AND suppliers.supplier_id=supp_trans.supplier_id";
				...
				if (DB::num_rows($result) == 1) {
					...
					if (DB::num_rows($result) > 0) {
						...
						return Errors::show_db_error("Invalid supptrans details for supptrans number : $trans_no and type : $trans_type",
							$sql, true);
					}
				} else {
		...
		public static function copy_to_trans($supp_trans)
			{
				...
				if (count($supp_trans->grn_items) > 0) {
					foreach ($supp_trans->grn_items as $grn) {
						$supp_trans->ov_amount += Num::round(($grn->this_quantity_inv * $grn->chg_price * (1 - $grn->discount / 100)),
							User::price_dec());
					}
				}
		...
		public static function totals($supp_trans)
			{
				...
				if ($supp_trans->is_invoice) {
					label_row(_("Invoice Total:"), Num::price_format($total), "colspan=$colspan align=right style='font-weight:bold;'",
					 "align=right id='invoiceTotal' data-total=" . $total . " style='font-weight:bold;'");
				} else {
					label_row(_("Credit Note Total"), Num::price_format($total),
