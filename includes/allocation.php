 ...
 */
		...
		function write() {
			...
			foreach ($this->allocs as $alloc_item)
			{
				if ($alloc_item->current_allocated > 0) {
					...
					Banking::exchange_variation($this->type, $this->trans_no,
						$alloc_item->type, $alloc_item->type_no, $this->date_,
	...
	function show_allocatable($show_totals) {
		...
		if (count($_SESSION['alloc']->allocs)) {
			...
			foreach ($_SESSION['alloc']->allocs as $alloc_item)
			{
				...
				label_cell("<a href='javascript:' name=Alloc$counter onclick='allocate_all(this.name.substr(5));return true;'>"
					 . _("All") . "</a>");
				label_cell("<a href='javascript:' name=DeAll$counter onclick='allocate_none(this.name.substr(5));return true;'>"
	...
	function create_miscorder($customer, $branch, $memo, $ref, $amount, $discount = 0) {

		processing_start();
		$type = ST_SALESINVOICE;
		$doc = new Cart(ST_SALESINVOICE, 0);
		$customer_error = get_customer_details_to_order($doc, $customer, $branch);
		$doc->trans_type = $type;
