<?php
	...
	class FB {
		 ...
		 * @var array
		 */
		protected $_json = array('version' => self::VERSION, 'columns' => array('label', 'log', 'backtrace', 'type'), 'rows' => array());
		 ...
		 * @var array
		 */
		protected $_settings = array(self::LOG_PATH => null, self::URL_PATH => null, self::STORE_LOGS => false, self::BACKTRACE_LEVEL => 1, self::MAX_TRANSFER => 3000);
		...
		protected function _writeToFile($json) {
			...
			$data =
			 array('uri' => $this->getSetting(self::URL_PATH) . '/' . $file_name, 'request_uri' => $_SERVER['REQUEST_URI'], 'time' => $this->_timestamp, 'version' => self::VERSION);
