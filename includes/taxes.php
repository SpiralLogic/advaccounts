<?php
	...
	class Taxes {
		...
		//
		static function get_tax_free_price_for_item($stock_id, $price, $tax_group, $tax_included, $tax_group_array = null) {
			...
			$tax_array = static::get_taxes_for_item($stock_id, $ret_tax_array);
		...
		//
		static function get_tax_for_item($stock_id, $price, $tax_group = false) {
			// if price is zero, then can't be taxed !
			if ($price == 0) return 0;
			if (!$tax_group) $tax_group = Tax_ItemType::get_for_item($stock_id);
			// if array already read, then make a copy and use that
		...
		static function get_full_price_for_item($stock_id, $price, $tax_group, $tax_included, $tax_group_array = null) {
			// if price is zero, then can't be taxed !
			if ($price == 0) return 0;
		...
		static function get_taxes_for_item($stock_id, $tax_group_items_array) {
		...
		static function get_tax_for_items($items, $prices, $shipping_cost, $tax_group, $tax_included = null, $tax_items_array = null) {
			...
			for ($i = 0; $i < count($items); $i++) {
				$item_taxes = static::get_taxes_for_item($items[$i], $ret_tax_array);
				if ($item_taxes != null) {
					foreach ($item_taxes as $item_tax) {
						$index = $item_tax['tax_type_id'];
						if ($tax_included == 1) { // 2008-11-26 Joe Hunt Taxes are stored without roundings
							$nprice = static::get_tax_free_price_for_item($items[$i], $prices[$i], $tax_group, $tax_included);
		...
		static function is_tax_account($account_code) {
