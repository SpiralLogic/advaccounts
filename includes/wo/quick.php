<?php
	...
	class WO_Quick
	{
		public static function add($wo_ref, $loc_code, $units_reqd, $stock_id, $type, $date_, $memo_, $costs, $cr_acc, $labour, $cr_lab_acc)
		{
			...
			WO_WorkOrder::add_overhead($stock_id, $units_reqd, $date_, $costs);
			if (!isset($labour) || ($labour == "")) {
				$labour = 0;
			}
			WO_WorkOrder::add_labour($stock_id, $units_reqd, $date_, $labour);
		...
		public static function costs($woid, $stock_id, $units_reqd, $date_, $advanced = 0, $costs = 0, $cr_acc = "", $labour = 0, $cr_lab_acc = "")
		{
			...
			if ($advanced) {
				...
				if ($issue_total != 0) {
					WO_WorkOrder::add_issue($stock_id, $units_reqd, $date_, $issue_total);
				}
				$lcost = WO_WorkOrder::get_gl($woid, WO_LABOUR);
				WO_WorkOrder::add_labour($stock_id, $units_reqd, $date_, $lcost * $units_reqd / $wo['units_reqd']);
				$ocost = WO_WorkOrder::get_gl($woid, WO_OVERHEAD);
				WO_WorkOrder::add_overhead($stock_id, $units_reqd, $date_, $ocost * $units_reqd / $wo['units_reqd']);
			}
			...
			if ($costs != 0.0) {
				...
				if ($is_bank_to) {
					Bank_Trans::add(ST_WORKORDER, $woid, $is_bank_to, "", $date_, -$costs, PT_WORKORDER, WO_OVERHEAD, Banking::get_company_currency(), "Cannot insert a destination bank transaction");
				}
			...
			if ($labour != 0.0) {
				...
				if ($is_bank_to) {
					Bank_Trans::add(ST_WORKORDER, $woid, $is_bank_to, "", $date_, -$labour, PT_WORKORDER, WO_LABOUR, Banking::get_company_currency(), "Cannot insert a destination bank transaction");
				}
