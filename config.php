<?php
 ...
 Config::set('debug.sql', false);
 Config::set('debug.pdf', false);
  Config::set('debug.query.log', false);
  Config::set('debug.select.log', false);
 ...
 Config::set('ui.windows.popups', true);
 /* use date picker for all date fields */
 Config::set('ui.forms.datepicker', true);
 /* use Audit Trails in GL */
 Config::set('logs.audits', true);
 /* use old style convert (income and expense in BS, PL) */
 Config::set('accounts.gl.oldconvertstyle', false);
 /* show users online discretely in the footer */
 Config::set('ui.users.showonline', true);
 /* default print destination. false = PDF/Printer, true = Excel */
 Config::set('print.default.excel', false);
 ...
 Config::set('accounts.datesystem', null);
 $date_system = 0;
 /* email stock location if order below reorder-level */
 Config::set('accounts.stock.emailnotify', null);
 $loc_notification = 0;
 /* print_invoice_no. 0 = print reference number, 1 = print invoice number */
 Config::set('print.useinvoicenumber', true);
 $print_invoice_no = 0;
 Config::set('ui.date.formats', array("MMDDYYYY", "DDMMYYYY", "YYYYMMDD"));
 $dateformats = array("MMDDYYYY", "DDMMYYYY", "YYYYMMDD");
 Config::set('ui.date.seperators', array("/", ".", "-", " "));
 $dateseps = array("/", ".", "-", " ");
 Config::set('ui.number.thousandseperators', array("/", ".", "-", " "));
 $thoseps = array(",", ".", " ");
 Config::set('ui.date.seperators', array("/", ".", "-", " "));
 $decseps = array(".", ",");
 // defalt dateformats and dateseps indexes used before user login
 Config::set('ui.date.format', 0);
 $dflt_date_fmt = 0;
 Config::set('ui.date.seperator', 0);
 $dflt_date_sep = 0;
 Config::set('print.pagesizes', array("Letter", "A4"));
 ...
 Config::set('logs.error.file', dirname(__FILE__) . '/tmp/errors.log');
