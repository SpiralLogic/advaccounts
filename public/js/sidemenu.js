...
{
  var $current, Searchboxtimeout, menuTimeout, Adv = window.Adv, sidemenu = {}, searchInput = $('<input/>').attr({type:'text', class:'small'}).data({'id':'', url:''}), $search = $("#search"), $quickCustomer = $('#quickCustomer'), $quickSupplier = $('#quickSupplier');
  (function ()
  {
    ...
    {
      $this.menu.unbind('mouseenter').accordion("disable");
    ...
    $search.delegate('input', "change blur keyup paste", function (event)
    {
      ...
      if (event.type != 'keyup') {
        menuTimeout = window.setTimeout($this.sidemenuOn, 1000);
      }
    });
