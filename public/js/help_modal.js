...
Adv.extend({ Help:(function () {
    var $current, showHelp = function () {
        var
          ...
          if (!$current.attr('id')) return;
        $.post(url, data, makePopover, 'json');
      },indicator=$('#help_indicator').on('click',showHelp);
      Adv.o.wrapper.on('mouseenter',':input',function(){$current=$(this); $(':input').popover('destroy');indicator.show().position({my:"left bottom", at:"right top", of:$current})});
  }())});
