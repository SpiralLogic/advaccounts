<?php
	...
	// or ui_msgs::display_warning(print_document_link($order_no, _("Print this order")));
	...
	function print_document_option_array($type_no, $doc_no, $email = 0, $extra = 0) {
		...
		switch ($type_no) {
			case ST_SALESQUOTE :
				...
				$ar = array('PARAM_0' => $doc_no, 'PARAM_1' => $doc_no, 'PARAM_2' => '', 'PARAM_3' => $email, 'PARAM_4' => '');
				break;
			case ST_SALESORDER :
				$rep = 109;
				// from, to, currency, bank acc, email, quote, comments
				$ar = array('PARAM_0' => $doc_no, 'PARAM_1' => $doc_no, 'PARAM_2' => '', 'PARAM_3' => $email, 'PARAM_4' => 0, 'PARAM_5' => '');
				break;
			case ST_CUSTDELIVERY :
				$rep = 110;
				// from, to, email, comments
				$ar = array('PARAM_0' => $doc_no, 'PARAM_1' => $doc_no, 'PARAM_2' => $email, 'PARAM_3' => $extra);
				break;
			...
			case ST_CUSTCREDIT : // Customer Credit Note
				...
				$ar = array('PARAM_0' => $doc_no, 'PARAM_1' => $doc_no, 'PARAM_2' => '', 'PARAM_3' => $email, 'PARAM_4' => '', 'PARAM_5' => '', 'PARAM_6' => $type_no);
				break;
			case ST_PURCHORDER :
				$rep = 209;
			...
			case ST_CUSTPAYMENT :
				$rep = 112;
				// from, to, currency, bank acc, email, comments
				$ar = array('PARAM_0' => $doc_no, 'PARAM_1' => $doc_no, 'PARAM_2' => '', 'PARAM_4' => '');
				break;
			case ST_CUSTREFUND :
				$rep = 113;
				// from, to, currency, bank acc, email, comments
			...
			case ST_PROFORMA :
				$rep = 129;
				// from, to, currency, bank acc, email, comments
				$ar = array('PARAM_0' => $doc_no, 'PARAM_1' => $doc_no, 'PARAM_2' => '', 'PARAM_3' => $email, 'PARAM_4' => '2');
				break;
			case ST_PROFORMAQ :
				$rep = 131;
				// from, to, currency, bank acc, email, comments
				$ar = array('PARAM_0' => $doc_no, 'PARAM_1' => $doc_no, 'PARAM_2' => '', 'PARAM_3' => $email, 'PARAM_4' => '3');
				break;
			case ST_SUPPAYMENT :
				$rep = 210;
				// from, to, currency, bank acc, email, comments
	...
	function email_document_link($doc_no, $link_text, $link = true, $type_no, $class = 'EmailLink', $id = '', $emails = array(), $extra = 0, $return = false) {
		if (empty($emails)) return false;
		if ($return) {
			ob_start();
		}
		$url = '/reporting/prn_redirect.php?';
		$options = print_document_option_array($type_no, $doc_no, 1, $extra);
		$ars = $options[0];
		$rep = $options[1];
		...
		UI::button('EmailButton' . $type_no, $link_text, array('style' => 'margin:20px', 'data-url' => $url))->p;
		$js = <<<JS
