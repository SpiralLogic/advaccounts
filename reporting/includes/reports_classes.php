	...
	class BoxReports {
		...
		function getDisplay($class = null) {
			...
			foreach ($this->ar_classes as $key => $value) {
				...
				$st_classes .= "<a href='" . $_SERVER['PHP_SELF'] . "?Class=$class_counter'" . " class='menu_option' id='" . ui_view::default_focus() . "'" . " onclick='return showClass($class_counter);'$acc[1]>$acc[0]</a> <br>";
				...
				foreach ($value as $report) {
					...
					$st_reports .= "<tr><td><a class='printlink' href='" . $_SERVER['PHP_SELF'] . "?Class=$class_counter&rep_id=$report->id'" . " id='" . ui_view::default_focus() . "'" . "$acc[1]>$acc[0]</a><tr><td>\n";
					if (isset($_REQUEST['rep_id']) && $_REQUEST['rep_id'] == $report->id) {
						...
						$st_params .= "\n</form></td></tr></table>\n";
						ui_view::set_focus('Rep' . $report->id);
...
//					return combo_input($name, '', $sql, 'id', 'name',array('spec_option'=>_("No Account Group Filter"),'spec_id'=>ALL_NUMERIC));
	...
	function gl_systypes_list($name, $value = null, $spec_opt = false) {
		...
		foreach (array(ST_LOCTRANSFER, ST_PURCHORDER, ST_SUPPRECEIVE, ST_MANUISSUE, ST_MANURECEIVE, ST_SALESORDER, ST_SALESQUOTE, ST_DIMENSION) as $type) unset($types[$type]);

		return array_selector($name, $value, $types, array('spec_option' => $spec_opt, 'spec_id' => ALL_NUMERIC, 'async' => false,));
	}
