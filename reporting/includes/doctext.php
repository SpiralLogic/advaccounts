<?php
	...
	if (isset($header2type) || isset($linetype)) {
		$doc_Invoice_no = ($doctype == ST_CUSTDELIVERY ? _("Delivery Note No.") : ($doctype == ST_CUSTPAYMENT ? _("Receipt No.") : ($doctype == ST_CUSTREFUND ? _("Refund No.")
		 : ($doctype == ST_SUPPAYMENT ? _("Remittance No.") : ($doctype == ST_PURCHORDER ? _("Purchase Order #:") : ($doctype == ST_SALESORDER ? _("Order No.") : ($doctype == ST_SALESQUOTE
			? _("Quotation No.") : ($doctype == ST_SALESINVOICE ? _("Invoice No.") : ($doctype == ST_WORKORDER ? _("Work Order No.") : _("Credit No."))))))))));
