<?php
	...
	class FrontReport extends Spreadsheet_Excel_Writer_Workbook {
		...
		function FrontReport($title, $filename, $size = 'A4', $fontsize = 9, $orientation = 'P', $margins = NULL, $excelColWidthFactor = 6.5) {
			...
			//$this->setCountry(48);
			if ($this->code != "iso-8859-1") $this->setVersion(8); // set biff version to 8 (0x0006 internal)
			$this->sheet =& $this->addWorksheet($this->worksheetNameGenerator($this->title));
			if ($this->code != "iso-8859-1") $this->sheet->setInputEncoding($this->code); // set sheet encoding
			if ($rtl) $this->sheet->setRTL();
		...
		function worksheetNameGenerator($name) {
			...
			$illegal_chars = array(':', '\\', '/', '?', '*', '[', ']');
			for ($i = 0; $i < count($illegal_chars); $i++) $name = str_replace($illegal_chars[$i], '', $name);
		...
		function NumFormat($dec) {
			if (!isset($this->formatAmount[$dec])) {
				...
				if ($dec > 0) $format .= "{$dsep}" . str_repeat('0', $dec);
		...
		function Info($params, $cols, $headers, $aligns, $cols2 = null, $headers2 = null, $aligns2 = null) {
			...
			if ($tcols > $this->numcols) $this->numcols = $tcols;
			for ($i = 0; $i < $this->numcols; $i++) $this->sheet->setColumn($i, $i, $this->px2units($this->cols[$i + 1] - $this->cols[$i]));
		}

		function Header() {
			$tcol = $this->numcols - 1;
			$this->sheet->setRow($this->y, 20);
			for ($i = 0; $i < $this->numcols; $i++) $this->sheet->writeBlank($this->y, $i, $this->formatTitle);
			...
			for ($i = 1; $i < count($this->params); $i++) {
				if ($this->params[$i]['from'] != '') {
					...
					if ($this->params[$i]['to'] != '') $str .= " - " . $this->params[$i]['to'];
					$this->sheet->writeString($this->y, 1, $str, $this->formatLeft);
					if ($i == 1) {
			...
			if ($this->headers2 != null) {
				for ($i = 0, $j = 0; $i < $this->numcols; $i++) {
					if ($this->cols2[$j] >= $this->cols[$i] && $this->cols2[$j] <= $this->cols[$i + 1]) {
						if ($this->aligns2[$j] == "right") $this->sheet->writeString($this->y, $i, $this->headers2[$j], $this->formatHeaderRight); else
			...
			for ($i = 0; $i < $this->numcols; $i++) {
				if (!isset($this->headers[$i])) $header = ""; else
					$header = $this->headers[$i];
				if ($this->aligns[$i] == "right") $this->sheet->writeString($this->y, $i, $header, $this->formatHeaderRight); else
		...
		function Header3() {
			...
			if (count($this->params) > 3) if ($this->params[3]['from'] != '') {
				$this->NewLine();
			...
			// - only print if available and not blank
			if (count($this->params) > 4) if ($this->params[4]['from'] != '') {
				$this->NewLine();
			...
			$this->sheet->writeString($this->y, 0, $str, $this->formatLeft);
			$str = '';
			if ($this->params[1]['from'] != '') $str = $this->params[1]['from'] . ' - ';
			...
			if ($this->headers2 != null) {
				for ($i = 0, $j = 0; $i < $this->numcols; $i++) {
					if ($this->cols2[$j] >= $this->cols[$i] && $this->cols2[$j] <= $this->cols[$i + 1]) {
						if ($this->aligns2[$j] == "right") $this->sheet->writeString($this->y, $i, $this->headers2[$j], $this->formatTopHeaderRight); else
			...
			for ($i = 0; $i < $this->numcols; $i++) {
				if (!isset($this->headers[$i])) $header = ""; else
					$header = $this->headers[$i];
				if ($this->aligns[$i] == "right") if ($this->headers2 == null) $this->sheet->writeString($this->y, $i, $header, $this->formatHeaderRight); else
					$this->sheet->writeString($this->y, $i, $header, $this->formatBottomHeaderRight); else
					if ($this->headers2 == null) $this->sheet->writeString($this->y, $i, $header, $this->formatHeaderLeft); else
		...
		function DatePrettyPrint($date, $input_format = 0, $output_format = 0) {
			if ($date != '') {
				...
				if ($output_format == 0) return (date('F j, Y', mktime(12, 0, 0, $month, $day, $year))); elseif ($output_format == 1) return (date('F Y',
					mktime(12, 0, 0, $month, $day,
						$year))); elseif ($output_format == 2) return (date('M Y',
					mktime(12,
						0,
						0,
						$month,
		...
		function TextCol($c, $n, $txt, $corr = 0, $r = 0, $border = 0, $fill = 0, $link = NULL, $stretch = 0) {
			...
			if ($this->aligns[$c] == 'right') $this->sheet->writeString($this->y, $c, $txt, $this->formatRight); else
				$this->sheet->writeString($this->y, $c, $txt, $this->formatLeft);
			if ($n - $c > 1) $this->sheet->mergeCells($this->y, $c, $this->y, $n - 1);
		}
		...
		function DateCol($c, $n, $txt, $conv = false, $corr = 0, $r = 0, $border = 0, $fill = 0, $link = NULL, $stretch = 0) {
			if (!$conv) $txt = Dates::date2sql($txt);
		...
		{ // if we need the time part too, we have to add this value after a decimalpoint.
			...
			$MAXYEAR = 2075;
			if (($year % 4) == 0) $mo[2]++;
			if ($mon < 1) $mon = 1; elseif ($mon > 12) $mon = 12;
			if ($day < 1) $day = 1; elseif ($day > $mo[$mon]) $day = $mo[$mon];
			if ($year < $BASE) $year = $BASE; elseif ($year > $MAXYEAR) $year = $MAXYEAR;
			$jul = (int)$day;
			for ($n = 1; $n < $mon; $n++) {
				$jul += $mo[$n];
			}
		...
		function End($email = 0, $subject = null, $myrow = null, $doctype = 0) {
			for ($i = 0; $i < $this->numcols; $i++) $this->sheet->writeBlank($this->y, $i, $this->formatFooter);
