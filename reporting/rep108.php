<?php
	...
	* ********************************************************************* */
	...
	//----------------------------------------------------------------------------------------------------
	function
	getTransactions($debtorno, $date, $payments =false) {
		...
		$sql .= " ORDER BY " . TB_PREF . "debtor_trans.branch_code, " . TB_PREF . "debtor_trans.tran_date";
	...
	function print_statements() {
		...
		while ($myrow = db_fetch($result)) {
			...
			for ($i = 0; $i < count($transactions); $i++) {
				...
				else $rep->TextCol(8, 9, -1 * $DisplayNet, -2);
				$rep->NewLine();
			...
			$str2 = array(number_format2(($CustomerRecord["Balance"] - $CustomerRecord["Due"]), $dec), number_format2(($CustomerRecord["Due"] - $CustomerRecord["Overdue1"]), $dec),
			              number_format2(($CustomerRecord["Overdue1"] - $CustomerRecord["Overdue2"]), $dec), number_format2($CustomerRecord["Overdue2"], $dec),
