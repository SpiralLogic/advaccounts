<?php
  ...
  class Sales extends Menu {
    ...
    protected function before() {
      ...
      $module->addRightFunction(_('&Invoice a Delivery'), '/sales/search/deliveries?OutstandingOnly=1', SA_SALESINVOICE);
      $module->addRightFunction(_('Invoice from &Template '), '/sales/search/orders?InvoiceTemplates=Yes', SA_SALESINVOICE);

      $module = $this->add_module(_('Payments & Inquiries'));
      ...
      $module->addLeftFunction(_('&Allocate Payments and Credits'), '/sales/allocations/customer_allocation_main?', SA_SALESALLOC);

      $module->addRightFunction(_('Search Invoices & Deliveries'), '/sales/search/transactions?', SA_SALESTRANSVIEW);
