<?php
  ...
  class Statement extends \ADV\App\Controller\Action
  {
    ...
    protected function index() {
      $this->Page->start(_($help_context = "Reconcile TO Bank Statement Compare"), SA_RECONCILE);
    ...
    protected function getUpload() {
      ...
      while (($item = fgetcsv($file, 1000, ',')) !== false) {
        ...
        try {
          ...
          return $this->status->set('false','Could not search current statement entries!');
        }
      }
      fclose($file);
      unlink($filename);
      return $this->status->set(true,'Bank statement successfully uploaded, records inserted: '.$inserted);
    }
  }
