  ...
  */
  ...
  class Sales_Order
  {
    ...
    public function check_cust_ref($cust_ref) {
      $sql    = "SELECT customer_ref,type FROM sales_orders WHERE debtor_id=" . DB::_escape($this->debtor_id) . " AND customer_ref=" . DB::_escape(
        $cust_ref
    ...
    public function add() {
      ...
      $sql
        ...
        $this->Branch
      ) . ", " . DB::_escape($this->cust_ref) . "," . DB::_escape($this->reference) . "," . DB::_escape($this->salesman) . "," . DB::_escape($this->Comments) . "," . DB::_escape(
        $this->source_no
      ) . ",'" . Dates::_dateToSql($this->document_date) . "', " . DB::_escape($this->sales_type) . ", " . DB::_escape($this->ship_via) . "," . DB::_escape(
        $this->deliver_to
      ) . "," . DB::_escape($this->delivery_address) . ", " . DB::_escape($this->name) . ", " . DB::_escape($this->phone) . ", " . DB::_escape($this->email) . ", " . DB::_escape(
        $this->freight_cost
      ...
      foreach ($this->line_items as $position => $line) {
        ...
        $sql .= DB::_escape(
          $line->id ? $line->id : 0
                        ...
                        " . DB::_escape($line->description) . ", " . DB::_escape($line->price) . ", " . DB::_escape($line->quantity) . ", " . DB::_escape(
    ...
    public function summary($title, $editable_items = false) {
      ...
      foreach ($this->line_items as $line_no => $stock_item) {
        $line_total    = round($stock_item->qty_dispatched * $stock_item->price * (1 - $stock_item->discount_percent), User::_price_dec());
        $line_discount = round($stock_item->qty_dispatched * $stock_item->price, User::_price_dec()) - $line_total;
        $qoh_msg       = '';
      ...
      if ($this->trans_type != 30 && !DB_Company::_get_pref('allow_negative_stock')) {
    ...
    public function header($date_text, $display_tax_group = false) {
      ...
      if (!$editable) {
        if (isset($this)) {
          ...
          Table::label(_('Customer:'), $this->customer_name . " - " . $this->deliver_to, "id='debtor_id_label' class='label pointer'");
        ...
        if (($this->Branch != Input::_post('branch_id', null, -1))) {
          if (!isset($_POST['branch_id']) || !$_POST['branch_id']) {
            ...
            if (Input::_post('cash') !== $this->cash) {
              $_POST['cash'] = $this->cash;
              Ajax::_activate('delivery');
              Ajax::_activate('cash');
            } else {
              if ($this->trans_type == ST_SALESINVOICE) {
                $_POST['delivery_date'] = $this->due_date;
                Ajax::_activate('delivery_date');
              }
      ...
      if ($editable) {
        ...
        Table::label(_("Reference:"), $this->reference);
      }
      ...
      if ($editable) {
        ...
        Table::label(_("Price List:"), $this->sales_type_name);
      }
      ...
      Table::label(_("Customer Discount:"), ($this->default_discount * 100) . "%");
      Table::section(4);
      if ($editable) {
        ...
        if (isset($_POST['_OrderDate_changed'])) {
          if (!Bank_Currency::is_company($this->customer_currency) && (DB_Company::_get_base_sales_type() > 0)) {
            $change_prices = 1;
          }
        ...
        Table::label($date_text, $this->document_date);
        Forms::hidden('OrderDate', $this->document_date);
      }
      if ($display_tax_group) {
        Table::label(_("Tax Group:"), $this->tax_group_name);
        Forms::hidden('tax_group_id', $this->tax_group_id);
      }
      Sales_UI::persons_row(_("Sales Person:"), 'salesman', (isset($this->salesman)) ? $this->salesman : User::_i()->salesmanid);
      Table::endOuter(1); // outer table
      if ($change_prices != 0) {
        foreach ($this->line_items as $line) {
          $line->price = Item_Price::get_kit($line->stock_id, $this->customer_currency, $this->sales_type, $this->price_factor, Input::_post('OrderDate'));
        }
        Ajax::_activate('items_table');
      }
