<?php
  ...
  class Bank_Account
  {
    ...
    public static function getBalances($bank_account, $begin_date, $end_date) {
      ...
      $sql           = "(select (rb - amount) as amount from temprec where date>=" . DB::quote($begin_date) . " and date<=" . DB::quote($end_date) . " and bank_account_id=" . DB::quote($bank_account) . " order by date, id desc limit 0,
          1) union (select rb as amount from temprec where date>=" . DB::quote($begin_date) . " and date<=" . DB::quote($end_date) . " and bank_account_id=" . DB::quote($bank_account) . " order by date desc, id asc limit 0,1)";
      $result        = DB::query($sql);
      $begin_balance = DB::fetch($result)['amount'];
      $end_balance   = DB::fetch($result)['amount'];
    ...
    public  static function getStatement($bank_account, $begin_date, $end_date) {
      $begin_date = static::checkBeginDate($begin_date);
      $sql        = "SELECT date as state_date, amount as state_amount,memo FROM temprec WHERE date >= " . DB::quote($begin_date) . " AND date <=" . DB::quote($end_date) . " and bank_account_id=" . DB::quote($bank_account) . " ORDER BY date ,amount";
      DB::query($sql);
      $statement_trans = DB::fetchAll();
