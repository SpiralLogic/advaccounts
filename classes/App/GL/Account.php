<?php
    ...
    class GL_Account
    {
        ...
        {
            $sql = "UPDATE chart_master SET account_name=" . DB::escape($account_name) . ",account_type=" . DB::escape($account_type) . ", account_code2=" . DB::escape(
                $account_code2
      ...
      SUM( g.amount ), bt.amount ) AS amount, bt.person_id, bt.person_type_id, bt.reconciled, bt.id
            ...
            WHERE   bt.bank_act = " . DB::quote($bank_account) . " AND bt.trans_date <= '" . Dates::dateToSql($date) . "' AND bt.undeposited<2
        ...
        {
            $sql = "UPDATE	reconciled FROM bank_trans WHERE bank_trans.bank_act = " . DB::escape($bank_account) . " AND undeposited = 0 AND reconciled = '" . Dates::dateToSql(
                $date
            ) . "'";
