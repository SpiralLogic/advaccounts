...
function handle_update_item()
{
	...
	if ($allow_update) {
		...
		$_SESSION['PO']->update_order_item(
			$_POST['line_no'], input_num('qty'), input_num('price'), $_POST['req_del_date'], $_POST['item_description'],
			$_POST['discount'] / 100);
		unset_form_variables();
	}
	line_start_focus();
		...
		if ($allow_update == true) {
			...
			if ($allow_update) {
				...
				$_SESSION['PO']->add_to_order($_POST['line_no'], $_POST['stock_id'], input_num('qty'),
											  $_POST['stock_id_text'], input_num('price'), $myrow["units"],
...
if (isset($_SESSION['PO']->supplier_id)) {
	...
	if (!empty($supplier_details['supp_address'])) echo '<span class="bold">Shipping Address:</span><br>' .
														$supplier_details['supp_address'] . '</br></br>';
	if (!empty($supplier_details['address'])) echo '<span class="bold">Mailing Address:</span><br>' .
												   $supplier_details['address'] . '</br></br>';
	if (!empty($supplier_details['phone'])) echo '<span class="bold">Phone: </span>' .
												 $supplier_details['phone'] . '</br></br>';
	if (!empty($supplier_details['phone2'])) echo '<span class="bold">Phone2: </span>' .
												  $supplier_details['phone2'] . '</br></br>';
	if (!empty($supplier_details['fax'])) echo '<span class="bold">Fax: </span>' .
											   $supplier_details['fax'] . '</br></br>';
	if (!empty($supplier_details['contact'])) echo '<span class="bold">Contact: </span>' .
												   $supplier_details['Contact'] . '</br></br>';
	if (!empty($supplier_details['email'])) echo '<span class="bold">Email: </span><a href="mailto:' .
												 $supplier_details['email'] . '">' .
												 $supplier_details['email'] . '</a></br></br>';
	if (!empty($supplier_details['website'])) echo '<span class="bold">Website: </span><a target="_new" href="http://' .
												   $supplier_details['website'] . '">' .
												   $supplier_details['website'] . '</a></br></br>';
