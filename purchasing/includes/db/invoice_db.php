<?php
	 ...
	 ***********************************************************************/
	include_once(APP_PATH . "purchasing/includes/db/invoice_items_db.php");
	...
	function add_supp_invoice($supp_trans, $invoice_no = 0) // do not receive as ref because we change locally
	{
		...
		foreach ($supp_trans->grn_items as $entered_grn)
		{
			...
			if ($supp_trans->is_invoice) {
				...
				$current_purchase_price = Price::getPriceBySupplier(Item::getStockID($entered_grn->item_code), $supp_trans->supplier_id)->price;

				if ($current_purchase_price == 0 || $current_purchase_price > $entered_grn->chg_price)
					add_or_update_purchase_data($supp_trans->supplier_id, $entered_grn->item_code, $entered_grn->chg_price);

				$deliveries = get_deliveries_between($entered_grn->item_code, $old_date, Dates::Today()); // extend the period, if invoice is before any deliveries.
	...
	function get_matching_invoice_item($stock_id, $po_item_id) {
		$sql = "SELECT *, tran_date FROM supp_invoice_items, supp_trans
