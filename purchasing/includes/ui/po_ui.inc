<?php
	...
	function display_po_header(&$order) {
		...
		if ($editable) {
			...
			if (isset($_POST['supplier_id'])) get_supplier_details_to_order($order, $_POST['supplier_id']);
		...
		if (!Banking::is_company_currency($order->curr_code)) {
			...
			ui_view::exchange_rate_display($order->curr_code, Banking::get_company_currency(),
				$_POST['OrderDate']);
		}
	...
	function display_po_items(&$order, $editable = true) {
		...
		label_cells(_("Total Excluding Shipping/Tax"), $display_total, "colspan=8 align=right",
	...
	function display_po_summary(&$po, $is_self = false, $editable = false) {
		...
		if (!$is_self) {
			label_cells(_("Purchase Order"), ui_view::get_trans_view_str(ST_PURCHORDER, $po->order_no),
				"class='tableheader2'");
		}
		end_row();
		start_row();
