	...
	$.extend(Calculator.prototype, {
		...
		_enableCalculator:function (target) {
			...
			if (nodeName == 'input') {
				...
				control.siblings('button.' + this._triggerClass).
				 each(
				 function () { this.disabled = false; }).end().
		...
		_disableCalculator:function (target) {
			...
			if (nodeName == 'input') {
				...
				control.siblings('button.' + this._triggerClass).
				 each(
				 function () { this.disabled = true; }).end().
				 siblings('img.' + this._triggerClass).
		...
		_generateHTML:function (inst) {
			...
			html = $(html);
			html.find('button').mouseover(
			 function () { $.calculator._saveClasses = this.className; }).
			 mousedown(
			 function () {
				 $(this).addClass('calculator-key-down' + (useTR ? ' ui-state-active' : ''));
			 }).
			 mouseup(
			 function () { $(this).removeClass().addClass($.calculator._saveClasses); }).
			 mouseout(
			 function () { $(this).removeClass().addClass($.calculator._saveClasses); }).
