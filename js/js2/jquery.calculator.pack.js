...
{ // hide the namespace
	...
	$.extend(Calculator.prototype, {
		...
		_attachCalculator: function(target, settings)
		{
			...
			if (inline) {
				$target.append(keyEntry).append(inst._mainDiv).
						bind('click.calculator', function()
				{
					keyEntry.focus();
				});
		...
		_generateHTML: function(inst)
		{
			...
			html = $(html);
			html.find('button').mousedown(
					function()
					{
						$(this).addClass('calculator-key-down');
					}).
					mouseup(
					function()
					{
						$(this).removeClass('calculator-key-down');
					}).
					mouseout(
					function()
					{
					...
					click(function()
						  {
							  $.calculator._handleButton(inst, $(this));
						  });
			return html;
		},
