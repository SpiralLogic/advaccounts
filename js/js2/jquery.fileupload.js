		...
		var fileUpload = this,
				...
				isXHRUploadCapable = function () {
					return typeof XMLHttpRequest !== undef && typeof File !== undef && (
							!settings.multipart || typeof FormData !== undef || (typeof FileReader !== undef && typeof XMLHttpRequest.prototype.sendAsBinary === func)
							);
				},
				...
				buildMultiPartFormData = function (boundary, files, filesFieldName, fields) {
					...
					$.each(fields, function (index, field) {
						formData += doubleDash + boundary + crlf + 'Content-Disposition: form-data; name="' + utf8encode(field.name) + '"' + crlf + crlf + utf8encode(field.value) + crlf;
					});
					$.each(files, function (index, file) {
						formData +=
								doubleDash + boundary + crlf + 'Content-Disposition: form-data; name="' + utf8encode(filesFieldName) + '"; filename="' + utf8encode(file
										                                                                                                                                    .name) + '"' + crlf + 'Content-Type: ' + utf8encode(file
												                                                                                                                                                                                        .type) + crlf + crlf + file
										.content + crlf;
					});
				...
				upload = function (event, files, index, xhr, settings) {
					...
					if (typeof settings.onSend === func && !settings.uploadedBytes && settings.onSend(event, files, index, xhr, settings) === false) {
						return;
					}
