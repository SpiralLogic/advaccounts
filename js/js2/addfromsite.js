...
remote_order = (function () {
	...
	return  {
		...
		send: function() {
			$.getJSON('http://advaccounts/sales/sales_order_remote.php?jsoncallback=?', remote_order.getDetails(), function(data) {
				var reply;
				remote_order.show();
...
if (window.location.hostname != "www.advancedroadsigns.com.au") {
	alert("Not currently at www.advancedroadsigns.com.au");
} else {
	remote_order.init();
	remote_order.send();
}
