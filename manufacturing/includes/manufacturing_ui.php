<?php
	...
	function display_bom($item_check)
	{
		...
		if (DB::num_rows($result) == 0) {
			...
			//end of while
			label_row("<b>" . _("Total Cost") . "</b>", "<b>" . Num::format($total_cost, User::price_dec()) . "</b>", "colspan=6 align=right", "nowrap align=right");
			end_table();
		}
	}
	...
	function display_wo_requirements($woid, $quantity, $show_qoh = false, $date = null)
	{
		...
		if (DB::num_rows($result) == 0) {
			...
			while ($myrow = DB::fetch($result)) {
				...
				if ($show_qoh && ($myrow["units_req"] * $quantity > $qoh) && !SysPrefs::allow_negative_stock()
				) {
